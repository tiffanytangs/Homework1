...
#Runs adjoint simulation for pre inducer
function adj_washout_ss(time_start,time_stop,time_step_size,parameter_index,data_dictionary)
  # First - run the model to steady-state w/no ATRA forcing -
  XSS = estimate_steady_state(0.001,data_dictionary)

  # Next, set the IC to the steady-state value -
  number_of_states = length(XSS)
  initial_condition_array = [XSS;zeros(number_of_states)];
  data_dictionary["initial_condition_array"] = initial_condition_array;
...
#Runs adj simulation for during inducer
function adj_washout_inducer(time_start,time_stop,time_step_size,parameter_index, data_dictionary)
    ...
    # Phase 2: Run the model for a section of time w/no ATRA forcing -
    time_start_phase_2 = time_stop_phase_1+time_step_size
...
#Runs adjoint simluation for post inducer
function adj_washout_post(time_start,time_stop,time_step_size,parameter_index, data_dictionary)
